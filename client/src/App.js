...
function App() {
  ...
  useEffect(() => {
    ...
    axios
      .post(
        "http://ec2-13-209-42-30.ap-northeast-2.compute.amazonaws.com:4000/kakao",
        {
      ...
      .then((kakaoUserData) => {
        if (kakaoUserData.data.kakao_account.email) {
          writeUserData(
            kakaoUserData.data.id,
            kakaoUserData.data.properties.nickname,
            "unknown",
            kakaoUserData.data.kakao_account.email
          );
          writeUserImageData(
            kakaoUserData.data.id,
            kakaoUserData.data.properties.profile_image
          );
        }
        // else {
        ...
        getUserData(kakaoUserData.data.id).then((snapshot) => {
          if (snapshot.exists()) {
