...
function Mypage() {
  ...
  // 유저 데이터 최신 상태로 받아오기
  useEffect(() => {
    getUserData(authCurrentUser.userId).then((snapshot) => {
      if (snapshot.exists()) {
  ...
  const imageHandler = (e) => {
    ...
    const saveRoute = storageRef.child(
      `${authCurrentUser.userId + "-" + imgFile[0].name}`
    );
    const upload = saveRoute.put(file);

    const imgUrl = `https://firebasestorage.googleapis.com/v0/b/personal-color-62f62.appspot.com/o/${
      authCurrentUser.userId + "-" + e.target.files[0].name
    }?alt=media`;
  ...
  return (
    <>
      ...
      <div className="mypage-box">
        ...
        <div className="mypage-edit-button" onClick={editUserinfoPopUpHandler}>
          내 정보 수정
        </div>
        {editUserinfoModalisOpen ? (
          <>
            <div className="modal-backdrop">
              <div className="modal">
                <span
                  onClick={editUserinfoPopUpHandler}
                ...
                <div
                  className="mypage-personal-edit-button"
                ...
                <div
                  ...
                  onClick={deleteUserMypageHandler}
                >
                  회원 탈퇴
                </div>
              </div>
        ...
        {authCurrentUser.color === undefined ? (
          <>
            <div className="mypage-personal-desc">
              {authCurrentUser.username}님의 등록된 컬러 정보가 없습니다. <br />{" "}
              진단을 통해 내 컬러 정보를 등록하러 가 볼까요?
            </div>
          </>
        ) : (
          <>
            <div className="mypage-personal-desc">
              {authCurrentUser.username}님의 퍼스널 컬러는
            </div>
            ...
            {seasons.map((season, i) => {
              if (authCurrentUser.color.indexOf(season) !== -1) {
                return (
                  <>
                    <div className="personal-color-desc">
                      {colorresult[i].desc}
                    </div>
                  </>
                );
            ...
            <div className="mypage-colorchip-box">
              {seasons.map((season, i) => {
                if (authCurrentUser.color.indexOf(season) !== -1) {
                  const color = colorresult[i]["recommend-color"].map(
                    (color) => {
                      return (
                        <div
                          className="mypage-color-chip"
